import { muapi } from '../../../packages/studio/src/backendClient.js';
import {
...
export function ImageStudio() {
    ...
    const addToHistory = (entry) => {
        ...
        localStorage.setItem('ai_cinema_history', JSON.stringify(generationHistory.slice(0, 50)));
        ...
        generationHistory.forEach((entry, idx) => {
            ...
            thumb.onclick = (e) => {
                if (e.target.closest('.hist-download')) {
                    downloadImage(entry.url, `ai-cinema-${entry.id || idx}.jpg`);
                    return;
                }
    ...
    try {
        const saved = JSON.parse(localStorage.getItem('ai_cinema_history') || '[]');
        if (saved.length > 0) {
    ...
    downloadBtn.onclick = () => {
        ...
        if (current) {
            ...
            downloadImage(current, `ai-cinema-${entry?.id || 'image'}.jpg`);
        }
    };
