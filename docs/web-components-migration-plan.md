...
## 14.1 P4 log (complete)

**P4 `studio-design`** — the design agent (Open-AI-Design-Agent `CreativeCanvas` + `CanvasArea` + `PlanVisualizer`, ~4.3k lines of React, the largest single remaining surface). Three new elements, all adopting the `design` sheet (`sheetKey='design'` → `public/wc/design.css`):

- **`src/wc/studio/studio-design.js`** (~2.4k lines) — `CreativeCanvas.jsx` port: sessions sidebar (new/rename/delete), top bar + profile dropdown, floating zoom toolbar, canvas resizer, chat sidebar (markdown bubbles w/ hljs code-gutter post-pass, `@`-mention popup for skills+assets, attach w/ XHR upload progress + hover preview, event pills: tool_call/tool_result/plan_propose approve-reject/info/error), creative-agent API client (`/api/v1/creative-agent`: sessions/messages/skills/jobs, 1.2 s poll w/ 6-min dead-air, final PATCH), initial handoff (`?q`/`?skill`/`?a`), asset→canvas sync, `setSearch()` for same-studio navigation.
- **`src/wc/studio/design-canvas-area.js`** (~1.95k lines) — `CanvasArea.jsx` react-konva → **plain imperative Konva**: `Stage/Layer/Rect/Image/Text/Transformer/Line/Group` rebuilt via an id-keyed `_sync()` diff (media cache, video frame-pump anim, audio meta/progress + play state, snap guides, transformer on the selected node). Public ref API kept 1:1 (`addImage/addVideo/addAudio/addNewText/getCanvasState/moveNode/placeNextToSource`/`replaceAt` alias/`arrangeNodes/zoomIn/zoomOut/resetZoom/handleZoomToFit/toggleAudio`) + wheel-zoom/pan, drag, HTML node & canvas context menus (export-as hover submenu, z-order, flip, lock/hide), text-edit textarea overlay, window `paste` + drop. Faithful quirk kept: audio drag-end not committed (snaps back, as the original).
- **`src/wc/studio/design-plan.js`** (142 lines) — `PlanVisualizer.jsx` port (topological plan DAG).

Porting notes:
- The `design` sheet **has** the theme `:root`/`.dark` variable blocks but, like every generated sheet, lacks the semantic token utilities (`bg-bg-page`, `text-primary-text`, …) — those classes were already inert in the React app (root-config generated CSS), so the native port keeps them inert too: appearance identical, and parity is text/DOM-based anyway.
- `useTheme()` (next-themes) outside a Provider is a no-op in the original → the profile "Dark Mode" button does nothing and the theme is forced `dark`; the port hard-codes both.
- DesignAgentStudio's synchronous pre-mount writes (`sessionStorage.fromDesignAgent`, `localStorage.token=apiKey`) are replicated in the element constructor; shell passes no props so the unauth path is the live one.
- **Shell search forwarding**: search-only navigations to an already-mounted native studio (the design agent's `?session=…` swaps) are forwarded to the element via `setSearch()` instead of remounting — mirrors React reconciliation. `justCreatedSession` flag consumption matches the original `sessionId` effect's early return; `main.js` now forwards `p.search` into `renderShell`.

Verification caught and fixed:
- Konva 10 has no `Node.z()` — `zIndex()` (and the transformer targets `children.length - 1`, not one past it, to avoid setZIndex range warnings).
- **`src/lib/icons.js` (P0) wrote `viewBox="24 24"`** — the `vb` values are `W H` pairs lacking the `0 0` prefix, i.e. an invalid SVG viewBox. Every native route logged 2–15 browser `<svg> viewBox` parse errors (icon content clipped to the top-left); invisible to the text/DOM parity probes, so this shipped across P0–P3. Fixed all 164 icons to `0 0 W H`.
- Two `cond && html`…`` in child position rendered literal `false` text nodes (the chat attachment/mention strip and the empty-streaming typing-dots branch) → `cond ? html`…`` : nothing`.

Live-verified: 20 buttons / 2 inputs / 1 canvas / shadowRoots 4; skills 404 `console.error` (fetch wording vs baseline AxiosError); full canvas ops loop (addImage/addNewText → getCanvasState → zoom in/out/reset/fit → arrangeNodes → z-order/flip/lock→deselect → duplicate/cut/paste/delete with node bookkeeping intact).
...
**Post-commit audit (clean-install + Docker)**: a clean `npm ci` inside the Docker frontend build caught a latent manifest gap — `konva` (imported directly by the P4 `design-canvas-area.js`) had only ever lived as a dependency of the deleted `design-agent` workspace, so it was undeclared in the root `package.json` and survived locally only in the stale node_modules. Added `konva@^10.3.1` as a root dependency (same minor as the deleted workspace used); full 19-route parity re-run green. Docker audit also found the `denoland/deno:1.44.0` runtime image is now **Debian bookworm** (upstream base change) — `RUN apk add` → `apt-get install` in the Deno stage. The apt step then failed locally with `Temporary failure resolving 'deb.debian.org'` — root-caused as a Docker/systemd-resolved interaction: the host resolves via the stub `127.0.0.53`, which is host-loopback and unreachable from containers, so Docker's built-in DNS forwarder gets NXDOMAIN; with `--network host` resolution succeeds. Fix per-environment: `docker build --network host` (or pin resolvable servers in the daemon's `dns` config); the Dockerfile itself is correct. The frontend-builder stage — everything the migration touched — builds clean in-image with `npm ci`. Housekeeping: deleted the three deleted-package `node_modules` skeletons left on disk, pruned root node_modules (780 MB → 73 MB; react/react-dom/reactflow gone), fixed the stale `npm run build:packages` step in `deno/README.md`. The 155 MB of `out/assets/*` in the image is pre-existing tracked demo media from `public/assets/` (vite copies `public/` into `out/`), not build bloat — the JS chunk itself is the single 1.86 MB file.
...
## 15. P7 log — vanilla CSS, minify-only build (complete)

Per request, the build pipeline is now **bundling + minification only, in production; dev never minifies** (both Vite defaults — `vite` dev serves unminified, `vite build` esbuild-minifies JS/CSS, nothing else runs).

**What changed**: all CSS is now committed plain-CSS artifacts, hand-maintained; every CSS generation tool is deleted.
...
**`npm run check:css` (new guard, `scripts/check-css.mjs`)** fails if a class used in any `class=` position (lit static + quoted dynamic fragments) has no rule in the committed CSS (artifacts ∪ element-local lit `css` blocks). Two tokenizer subtleties matter: Tailwind emits variant classes as single escaped names (`dark\:text-x`, `group-hover\/att\:opacity-100`, `.\32xl\:grid-cols-6` — leading-digit hex escape), so the defined-side scanner is escape-aware (CSS 1–6 hex escapes + terminating space; bare `:` stops the name at pseudo-classes); the used-side treats `:`/`/`/`=`/`#`/`()`/`,` as token characters (arbitrary values `shadow-[0_0_12px_rgba(…)]`, variants `data-[selected=true]:x`) rather than split boundaries. Result: **1,268 classes in use, all covered**.

**Allow-listed (77 classes, documented in the guard)** — classes that were **never styled in the original build either** (root Tailwind v3 ran over `src/` + `packages/studio` only; the vendored packages' theme tokens were never in the root config; no `tailwindcss-animate`/typography plugins; v3 has no dynamic numerics): package-token utilities (`bg-bg-page/50`, `text-primary-text`, `border-divider`, `dark:bg-primary-bg/95`, …), non-theme values (`h-18`, `w-18`, `h-22`, `left-4.5`, `rotate-270`, `shadow-4xl`, `hover:scale-80`, `*-zinc-650/705/850/905`), plugin classes (`animate-in`, `fade-in`, `zoom-in*`, `slide-in-from-*`, `prose*`), and port marker/JS-hook classes (`canvas-container`, `ctx-menu`, `no-scrollbar`, `progress-bar`, `group/user-att`, …). Deliberately left unstyled = original parity.
